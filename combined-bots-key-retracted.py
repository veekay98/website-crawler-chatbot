...
def generate_direct_response(query):
    # Prepare the prompt for GPT-4 without using the knowledge base
...
@app.route('/')
def index():
    return render_template('combined-bots.html')
...
@app.route('/direct-gpt', methods=['POST'])
def direct_gpt_endpoint():
    user_input = request.json.get('query')
    user_name = request.json.get('name', 'User')

    # Generate a response using GPT-4's broader knowledge without the knowledge base
    response_text = generate_direct_response(user_input)
